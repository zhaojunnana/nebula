...
// static
StreamExecutor *StreamExecutor::makeStreamExecutor(
    const PlanNode *node,
    QueryContext *qctx,
    std::unordered_map<int64_t, StreamExecutor *> *visited,
...
int32_t StreamExecutor::markFinishTask(bool hasNextRound) {
  ...
  DLOG(INFO) << "markFinishTask " << id() << " with upStreamFinished:" << upStreamFinished
             << ", hasNextRound:" << hasNextRound << ", currentTaskCount:" << currentTaskCount;
