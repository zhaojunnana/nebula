...
std::shared_ptr<RoundResult> TraverseStreamExecutor::executeOneRound(std::shared_ptr<DataSet> input,
                                                                     Offset offset) {
  ...
  if (resultStatus.ok()) {
    auto roundResult =
        std::make_shared<RoundResult>(std::make_shared<DataSet>(std::move(roundCtx.result_)),
                                      roundCtx.hasNext_,
...
Status TraverseStreamExecutor::buildRequestVids(DataSet* input, TraverseRoundContext& roundCtx) {
  ...
  auto inputIter =
      ResultBuilder().value(Value(*input)).iter(Iterator::Kind::kSequential).build().iter();
...
folly::Future<Status> TraverseStreamExecutor::getNeighbors(Offset& offset,
                                                           TraverseRoundContext& roundCtx) {
  ...
  return storageClient
      ...
      .via(runner())
      .thenValue(
          [this, getNbrTime, &roundCtx](StorageRpcResponse<GetNeighborsResponse>&& resp) mutable {
...
void TraverseStreamExecutor::addStats(RpcResponse& resp,
                                      int64_t getNbrTimeInUSec,
                                      TraverseRoundContext& roundCtx) {
...
folly::Future<Status> TraverseStreamExecutor::asyncExpandOneStep(RpcResponse&& resps,
                                                                 TraverseRoundContext& roundCtx) {
  ...
  return folly::collect(futures).via(runner()).thenValue(
      [this, initVerticesList, vidsList, adjLists, taskRunTime, &roundCtx](
        ...
        for (auto& initVertices : *initVerticesList) {
          std::move(
              initVertices.begin(), initVertices.end(), std::back_inserter(roundCtx.initVertices_));
        }
...
folly::Future<Status> TraverseStreamExecutor::handleResponse(RpcResponse&& resps,
                                                             TraverseRoundContext& roundCtx) {
...
folly::Future<Status> TraverseStreamExecutor::buildPathMultiJobs(size_t minStep,
                                                                 size_t maxStep,
                                                                 TraverseRoundContext& roundCtx) {
  DataSet vertices;
