...
std::shared_ptr<RoundResult> ProjectStreamExecutor::executeOneRound(std::shared_ptr<DataSet> input,
                                                                    Offset offset) {
  // throw std::bad_alloc in MemoryCheckGuard verified
  SCOPED_TIMER(&execTime_);
  auto iter = ResultBuilder().value(Value(*input)).iter(Iterator::Kind::kProp).build().iter();
  DCHECK(!!iter);
