...
ClusterCursors IndexScanStreamExecutor::transToClusterCursors(Offset& offset) {
  ClusterCursors clusterCursors;
  // parse ip:post-indexId-partId key
  for (auto& entity : offset) {
    auto key = entity.first.getStr();
    std::vector<std::string> parts = splitString(key, ':');
    if (parts.size() == 2) {
        std::string ip = parts[0];
        std::vector<std::string> secondPart = splitString(parts[1], '-');
        if (secondPart.size() == 3) {
            std::string port = secondPart[0];
            std::string indexId = secondPart[1];
            std::string partId = secondPart[2];
            auto host = HostAddr(ip, std::stoi(port));
            Offset& cursors = clusterCursors[host];

            std::string cursorKeyStr = "";
            cursorKeyStr.append(indexId).append("-").append(partId);
            auto cursorKey = Value(cursorKeyStr);
            cursors.emplace(cursorKey, entity.second);
        }
    }
  }
  return clusterCursors;
}

// TODO(shylock) merge the handler with GetProp
template <typename Resp>
StatusOr<std::shared_ptr<RoundResult>> IndexScanStreamExecutor::handleResp(
  storage::StorageRpcResponse<Resp> &&rpcResp) {
  ...
  for (auto &resp : rpcResp.responses()) {
    auto [host, latency, e2eLatency] = rpcResp.hostLatency()[i++];
    ...
    auto cusrors = resp.get_cursors();
    for (auto it = cusrors.begin(); it != cusrors.end(); it++) {
      if (it->second.next_cursor_ref().has_value()) {
        auto cursorKeyStr = it->first.getStr();
        std::string clusterCursorKeyStr = "";
        clusterCursorKeyStr.append(host.toRawString()).append("-").append(cursorKeyStr);
        offset.emplace(Value(clusterCursorKeyStr), it->second);
        hasNext = true;
      }
    }
  }
