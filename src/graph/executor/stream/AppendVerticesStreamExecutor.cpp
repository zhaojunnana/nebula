...
std::shared_ptr<RoundResult> AppendVerticesStreamExecutor::executeOneRound(
  ...
  auto inputResult = ResultBuilder().value(Value(*input)).iter(Iterator::Kind::kProp).build();
  ...
  if (resultStatus.ok()) {
    auto roundResult = std::make_shared<RoundResult>(
        std::make_shared<DataSet>(std::move(roundCtx.result_)), false, offset);
    return roundResult;
  } else {
...
StatusOr<DataSet> AppendVerticesStreamExecutor::buildRequestDataSet(const AppendVertices *av,
                                                                    Result &input) {
  if (av == nullptr) {
...
folly::Future<Status> AppendVerticesStreamExecutor::appendVertices(
  ...
  return DCHECK_NOTNULL(storageClient)
      ...
      .via(runner())
      .thenValue(
          [this, getPropsTime, &input, &roundCtx](StorageRpcResponse<GetPropResponse> &&rpcResp) {
...
Status AppendVerticesStreamExecutor::handleNullProp(const AppendVertices *av,
                                                    Result &input,
                                                    AppendVerticesRoundContext &roundCtx) {
...
folly::Future<Status> AppendVerticesStreamExecutor::handleRespMultiJobs(
  ...
  if (!av->trackPrevPath()) {
    ...
    auto gather = [&roundCtx](std::vector<folly::Try<StatusOr<DataSet>>> &&results) -> Status {
      memory::MemoryCheckGuard guard;
    ...
    auto gather =
      ...
      auto scatterInput = [this, &roundCtx](size_t begin, size_t end, Iterator *tmpIter) mutable
          -> StatusOr<DataSet> { return handleJob(begin, end, tmpIter, roundCtx); };

      auto gatherFinal =
          [&roundCtx](std::vector<folly::Try<StatusOr<DataSet>>> &&results) -> Status {
...
DataSet AppendVerticesStreamExecutor::buildVerticesResult(size_t begin,
                                                          size_t end,
                                                          Iterator *iter) {
  auto *av = asNode<AppendVertices>(node());
  auto vFilter = av->vFilter() ? av->vFilter()->clone() : nullptr;
...
void AppendVerticesStreamExecutor::buildMap(size_t begin,
                                            size_t end,
                                            Iterator *iter,
                                            AppendVerticesRoundContext &roundCtx) {
  auto *av = asNode<AppendVertices>(node());
...
DataSet AppendVerticesStreamExecutor::handleJob(size_t begin,
                                                size_t end,
                                                Iterator *iter,
                                                AppendVerticesRoundContext &roundCtx) {
  auto *av = asNode<AppendVertices>(node());
