...
class TraverseStreamExecutor final : public StorageAccessStreamExecutor {
 public:
  ...
  std::shared_ptr<RoundResult> executeOneRound(std::shared_ptr<DataSet> input,
 ...
 private:
  ...
  std::vector<Row> buildPath(const Value& initVertex,
                             size_t minStep,
                             size_t maxStep,
                             TraverseRoundContext& roundCtx);
  folly::Future<Status> buildPathMultiJobs(size_t minStep,
                                           size_t maxStep,
                                           TraverseRoundContext& roundCtx);
  std::vector<Row> joinPrevPath(const Value& initVertex,
