...
bool StorageAccessStreamExecutor::isIntVidType(const SpaceInfo &space) const {
  return (*space.spaceDesc.vid_type_ref()).type == nebula::cpp2::PropertyType::INT64;
}

StatusOr<DataSet> StorageAccessStreamExecutor::buildRequestDataSetByVidType(Iterator *iter,
                                                                      Expression *expr,
                                                                      bool dedup,
                                                                      bool isCypher) {
...
StatusOr<std::vector<Value>> StorageAccessStreamExecutor::buildRequestListByVidType(Iterator *iter,
                                                                              Expression *expr,
                                                                              bool dedup,
                                                                              bool isCypher) {
  const auto &space = qctx()->rctx()->session()->space();
...
bool StorageAccessStreamExecutor::hasSameEdge(
  const std::vector<Value> &edgeList, const Edge &edge) {
...
folly::Future<std::vector<Value>> StorageAccessStreamExecutor::getProps(
...
std::vector<Value> StorageAccessStreamExecutor::handlePropResp(PropRpcResponse &&resps) {
...
void StorageAccessStreamExecutor::addGetNeighborStats(
  RpcResponse &resp, size_t stepNum, bool reverse) {
