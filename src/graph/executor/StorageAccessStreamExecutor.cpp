...
StatusOr<std::vector<Value>> StorageAccessStreamExecutor::buildRequestListByVidType(
    Iterator *iter, Expression *expr, bool dedup, bool isCypher) {
  const auto &space = qctx()->rctx()->session()->space();
  QueryExpressionContext exprCtx(qctx()->ectx());
...
bool StorageAccessStreamExecutor::hasSameEdge(const std::vector<Value> &edgeList,
                                              const Edge &edge) {
...
void StorageAccessStreamExecutor::addGetNeighborStats(RpcResponse &resp,
                                                      size_t stepNum,
                                                      bool reverse) {
