...
bool NebulaListener::applyBatch(const BatchHolder& batch) {
  ...
  auto executeWithRetry = [&](auto operation) -> bool {
    ...
    if (!lastException.empty()) {
      LOG(ERROR) << "Operation failed after " << maxRetries
                 << " retries with exception: " << lastException;
    }
  ...
  if (success && !deleteEdges.empty()) {
    if (!executeWithRetry([&]() { return storage_->deleteEdges(param, std::move(deleteEdges)); })) {
      success = false;
    }
  }
